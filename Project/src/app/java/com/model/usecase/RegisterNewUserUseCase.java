...
public class RegisterNewUserUseCase extends UseCase {
	...
	public RegisterNewUserUseCase(RegisterNewUserResultInterface resultInterface, User user,
			Account account) {
	...
	@Override
	/*
	 * if TEQ staff insert the user to User table, TEQStaff table if officer insert into user table,
	 * Agency table, officer table
	 */
	public void run() {
		String type = getType();
		if (type != null) {
	...
	private void insertOfficer(int agencyId) throws DuplicateKeyException, InsertException {
		...
		InsertCommand insertOfficerComm =
				new InsertCommand(officerTable, insertOfficerAttr, insertOfficerVal);
	...
	private int insertAgency() throws DuplicateKeyException, InsertException {
		...
		InsertCommand insertAgencyComm =
				new InsertCommand(agencyTable, insertAgencyAttr, insertAgencyVal);
	...
	private void registerAccount() throws Exception {
		...
		Command registerAccount = new UpdateCommand(accountTable, registerAccountAttrs,
				registerAccountVals, registerAccountCons);
